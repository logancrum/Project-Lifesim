...
// FIXME: This function might need to be deprecated due to the new: bool sellItem(int) (SEE DIRECTLY BELOW)
...
bool Inventory::sellItem(int indexArg) {
	if ((indexArg < carriedItems.size()) && (indexArg > 0)) {
		Item itemArg = carriedItems.at(indexArg);
		int cost = itemArg.getItemValue() * sellMultiplier;
		int change = 0;
...
int Inventory::getTotalPurse() {
	return ((copperBits * COPPER_VALUE)
...
void Inventory::printItemLineBuy(Item itemArg) {
	// Print the itemName and the amount on hand END WITH " | "
	std::cout << itemArg.getItemName() << " | " << carriedItemsAndAmounts[itemArg.getItemName()] << " | ";

	// Print the itemType and the damage/defense END WITH " | "
	std::cout << itemArg.getItemType() << " | ";
	// Two if statements: 
	//		Some items will have both Dam and DT ratings, e.g., spiked shields or large swords with crossguards
	if (itemArg.getItemDam() != 0) {
		std::cout << "Damage: " << itemArg.getItemDam() << " | ";
	}
	if (itemArg.getItemDT() != 0) {
		std::cout << "Defense: " << itemArg.getItemDT() << " | ";
	}

	// Print itemValue, END WITH NEWLINE
	std::cout << "Value: " << itemArg.getItemValue() * sellMultiplier << " |      " << std::endl;
}
...
void Inventory::printItemLineNeutral(Item itemArg) {
	...
	// Print itemValue, END WITH NEWLINE
	std::cout << "Value: " << itemArg.getItemValue() << " |      " << std::endl;
}
